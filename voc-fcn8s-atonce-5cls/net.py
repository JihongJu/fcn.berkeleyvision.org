...
def fcn(split):
    ...
    n.score_fr = L.Convolution(n.drop7, num_output=6, kernel_size=1, pad=0,
        weight_filler=dict(type='xavier'),
        param=[dict(lr_mult=1, decay_mult=1), dict(lr_mult=2, decay_mult=0)])
    n.upscore2 = L.Deconvolution(n.score_fr,
        convolution_param=dict(num_output=6, kernel_size=4, stride=2,
            weight_filler=dict(type='xavier'),
            bias_term=False),
    ...
    n.score_pool4 = L.Convolution(n.scale_pool4, num_output=6, kernel_size=1, pad=0,
        weight_filler=dict(type='xavier'),
        param=[dict(lr_mult=1, decay_mult=1), dict(lr_mult=2, decay_mult=0)])
    ...
    n.score_pool3 = L.Convolution(n.scale_pool3, num_output=6, kernel_size=1, pad=0,
        weight_filler=dict(type='xavier'),
        param=[dict(lr_mult=1, decay_mult=1), dict(lr_mult=2, decay_mult=0)])
    ...
    n.upscore8 = L.Deconvolution(n.fuse_pool3,
        convolution_param=dict(num_output=6, kernel_size=16, stride=8,
            weight_filler=dict(type='xavier'),
            bias_term=False),
        param=[dict(lr_mult=0)])
